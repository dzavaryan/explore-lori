/* stories-ui.js — Stories content management */
(function () {
  'use strict';

  const API     = '/api/stories';
  const panel   = document.getElementById('editPanel');
  const overlay = document.getElementById('overlay');
  const tbody   = document.getElementById('tableBody');

  const PILLARS = {
    trails: 'Trails & Nature',
    culture: 'Culture & History',
    food: 'Food & Foraging',
    people: 'Local Voices',
    seasons: 'Seasons & Weather',
  };

  let items = [];
  let editingId = null;

  const F = {
    title:      () => document.getElementById('fTitle'),
    slug:       () => document.getElementById('fSlug'),
    excerpt:    () => document.getElementById('fExcerpt'),
    author:     () => document.getElementById('fAuthor'),
    readTime:   () => document.getElementById('fReadTime'),
    pillar:     () => document.getElementById('fPillar'),
    season:     () => document.getElementById('fSeason'),
    image:      () => document.getElementById('fImage'),
    featured:   () => document.getElementById('fFeatured'),
    imgPreview: () => document.getElementById('fImgPreview'),
    imageUpload:() => document.getElementById('fImageUpload'),
  };

  async function loadItems() {
    try {
      items = await AdminCore.api('GET', '/stories');
      renderTable();
    } catch (e) {
      AdminCore.toast('Failed to load stories: ' + e.message, 'error');
    }
  }

  function renderTable() {
    tbody.innerHTML = '';
    items.forEach(item => {
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td>${item.title || '—'}</td>
        <td>${PILLARS[item.pillar] || item.pillar || '—'}</td>
        <td>${item.author || '—'}</td>
        <td>${item.readTime || '—'}</td>
        <td>${item.featured ? '★' : ''}</td>
        <td class="table-actions">
          <button class="btn btn--sm btn--secondary btn-edit" data-id="${item.id}">Edit</button>
        </td>`;
      tbody.appendChild(tr);
    });
    tbody.querySelectorAll('.btn-edit').forEach(btn => {
      btn.addEventListener('click', () => openEdit(btn.dataset.id));
    });
  }

  function openNew() {
    editingId = null;
    document.getElementById('panelTitle').textContent = 'New Story';
    document.getElementById('btnDelete').style.display = 'none';
    clearForm();
    AdminCore.openPanel(panel, overlay);
  }

  function openEdit(id) {
    const item = items.find(i => i.id === id);
    if (!item) return;
    editingId = id;
    document.getElementById('panelTitle').textContent = 'Edit Story';
    document.getElementById('btnDelete').style.display = '';
    fillForm(item);
    AdminCore.openPanel(panel, overlay);
  }

  function clearForm() {
    F.title().value = '';
    F.slug().value = '';
    F.excerpt().value = '';
    F.author().value = '';
    F.readTime().value = '';
    F.pillar().value = 'trails';
    F.season().value = 'spring';
    F.image().value = '';
    F.featured().checked = false;
    F.imgPreview().style.display = 'none';
    F.imgPreview().src = '';
  }

  function fillForm(item) {
    F.title().value    = item.title || '';
    F.slug().value     = item.slug || '';
    F.excerpt().value  = item.excerpt || '';
    F.author().value   = item.author || '';
    F.readTime().value = item.readTime || '';
    F.pillar().value   = item.pillar || 'trails';
    F.season().value   = item.season || 'spring';
    F.image().value    = item.image || '';
    F.featured().checked = !!item.featured;
    if (item.image) {
      F.imgPreview().src = item.image;
      F.imgPreview().style.display = 'block';
    } else {
      F.imgPreview().style.display = 'none';
    }
  }

  function collectForm() {
    const pillar = F.pillar().value;
    return {
      title:       F.title().value.trim(),
      slug:        F.slug().value.trim(),
      excerpt:     F.excerpt().value.trim(),
      author:      F.author().value.trim(),
      readTime:    F.readTime().value.trim(),
      pillar:      pillar,
      pillarLabel: PILLARS[pillar] || pillar,
      season:      F.season().value,
      image:       F.image().value.trim(),
      featured:    F.featured().checked,
    };
  }

  F.imageUpload().addEventListener('change', async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    const fd = new FormData();
    fd.append('image', file);
    try {
      const res = await fetch('/api/upload', { method: 'POST', body: fd });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Upload failed');
      F.image().value = data.path;
      F.imgPreview().src = data.path;
      F.imgPreview().style.display = 'block';
      AdminCore.toast('Image uploaded', 'success');
    } catch (e) {
      AdminCore.toast('Upload error: ' + e.message, 'error');
    }
  });

  F.image().addEventListener('input', () => {
    const v = F.image().value.trim();
    F.imgPreview().src = v;
    F.imgPreview().style.display = v ? 'block' : 'none';
  });

  async function saveItem(andRebuild) {
    const body = collectForm();
    if (!body.title) { AdminCore.toast('Title is required', 'error'); return; }
    try {
      if (editingId) {
        await AdminCore.api('PUT', '/stories/' + editingId, body);
      } else {
        await AdminCore.api('POST', '/stories', body);
      }
      AdminCore.toast('Saved!', 'success');
      AdminCore.closePanel(panel, overlay);
      await loadItems();
      if (andRebuild) {
        const res = await AdminCore.api('POST', '/build/stories');
        AdminCore.toast('Stories page rebuilt (' + (res.injected||0) + ' cards)', 'success');
      }
    } catch (e) {
      AdminCore.toast('Save error: ' + e.message, 'error');
    }
  }

  document.getElementById('btnDelete').addEventListener('click', async () => {
    if (!editingId) return;
    const ok = await AdminCore.confirm('Delete this story? This cannot be undone.');
    if (!ok) return;
    try {
      await AdminCore.api('DELETE', '/stories/' + editingId);
      AdminCore.toast('Deleted', 'success');
      AdminCore.closePanel(panel, overlay);
      await loadItems();
    } catch (e) {
      AdminCore.toast('Delete error: ' + e.message, 'error');
    }
  });

  document.getElementById('btnNew').addEventListener('click', openNew);
  document.getElementById('btnClose').addEventListener('click', () => AdminCore.closePanel(panel, overlay));
  overlay.addEventListener('click', () => AdminCore.closePanel(panel, overlay));
  document.getElementById('btnSave').addEventListener('click', () => saveItem(false));
  document.getElementById('btnSaveRebuild').addEventListener('click', () => saveItem(true));

  F.title().addEventListener('input', () => {
    if (!editingId) {
      F.slug().value = F.title().value
        .toLowerCase().trim()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '');
    }
  });

  loadItems();
})();
